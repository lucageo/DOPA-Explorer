...
var streets_   = L.tileLayer(mbUrl, {id: 'mapbox.light', attribution: 'October 2017 version of the World Database on Protected Areas (WDPA)'});//.addTo(lMap);
  ...
  lMap.on('zoomend', function() {
      if (lMap.getZoom() < 5){
             ...
             lMap.removeLayer(wdpa_hi);
              wdpa.setOpacity(1);
              wdpa.setParams({CQL_FILTER:"jrc_gis_area_km2 > 249"});


      } else if (lMap.getZoom() >= 5){
        lMap.addLayer(wdpa_hi);
        wdpa.setOpacity(0.8);
        wdpa.setParams({CQL_FILTER:"jrc_gis_area_km2 > 49"});
...
var query5 = "SELECT * FROM terrestrial_ecoregion_centroids limit 100";
...
// Check if the layer is WDPA
             ...
             var url = getFeatureInfoUrl(
                             ...
                             {
                                 'info_format': 'text/javascript',  //it allows us to get a jsonp
                                 'propertyName': 'wdpaid,marine,wdpa_name,desig_eng,desig_type,iucn_cat,jrc_gis_area_km2,rep_area,status,status_yr,mang_auth,country,iso3',
...
var query4 = "SELECT the_geom, wdpaid, name FROM dopa_carto_wdpa_centroids_master limit 10000";
...
wdpa.setParams({CQL_FILTER:"jrc_gis_area_km2 > 200"});
//------------------------------------------------------------------------------
...
var wdpa_hi=L.tileLayer.wms(url, {
 ...
 wdpa_hi.setParams({CQL_FILTER:"wdpa_name LIKE 'xxx'"}); // GEOSERVER WMS FILTER
...
var url_marine = 'http://dopa-services.jrc.ec.europa.eu/services/ibex/ehabitat/get_radarplot_pa?wdpaid=' + wdpaid; // get radar plot in pa
var url_coastal = 'http://dopa-services.jrc.ec.europa.eu/services/ibex/ehabitat/get_radarplot_pa?wdpaid=' + wdpaid; // get radar plot in pa
...
$('#infographic_title_threat').show();
$('#infographic_title_threat p').show();

$('#infographic_agr_title').show();
$('#infographic_bar_wdpa_general_agr').show();

$('#infographic_introad_title').show();
$('#infographic_bar_wdpa_general_introad').show();

$('#infographic_eroad_title').show();
$('#infographic_bar_wdpa_general_eroad').show();

$('#infographic_pop_title').show();
$('#infographic_bar_wdpa_general_pop').show();

$('#infographic_popc_title').show();
$('#infographic_bar_wdpa_general_popc').show();
        ...
        } // end of else (if you wat to add terrestrial and marine hdi is here a new if else)
      }
    });
  }else if (marine == '1'){
              ...
              $(d.records).each(function(i, data) {
                  ...
                  switch (data.title) {
                      ...
                      for (var prop in data) {
                          ...
                          for (var prop in data) {
                                  if (prop == 'title') {
                                      title.push(" Marine <br> Habitat <br> Diversity")
                                  }
                                  else if (prop == 'country_avg') {
                                      if(data[prop]>=0)
                                      country_avg.push(data[prop]);
...
$('#infographic_title_threat').hide();
$('#infographic_title_threat p').hide();

$('#infographic_agr_title').hide();
$('#infographic_bar_wdpa_general_agr').hide();

$('#infographic_introad_title').hide();
$('#infographic_bar_wdpa_general_introad').hide();

$('#infographic_eroad_title').hide();
$('#infographic_bar_wdpa_general_eroad').hide();

$('#infographic_pop_title').hide();
$('#infographic_bar_wdpa_general_pop').hide();

$('#infographic_popc_title').hide();
$('#infographic_bar_wdpa_general_popc').hide();
